...
def visualize_q2(taxi_trip_count):
    ...
    # 查找所有值是 1 的出租车
    # one_trip_taxi = taxi_trip_count[taxi_trip_count['count'] == 1]
    # print(one_trip_taxi)
    ...
    sns.histplot(taxi_trip_count['count'], bins=50, kde=True)
    ...
    # 绘制箱线图
    plt.figure(figsize=(10, 6))
    sns.boxplot(y=taxi_trip_count['count'])

    # 获取当前图形的坐标轴
    ax = plt.gca()

    # 标注订单数最多的 taxi_id
    ax.annotate(f'Max Taxi ID: {max_taxi_id}', 
                xy=(0, max_count), 
                xytext=(0, max_count + 20),  # 调整标注的位置
                arrowprops=dict(facecolor='black', shrink=0.05),
                ha='center')

    ax.annotate(f'Min Taxi ID: {min_taxi_id}',
                xy=(0, min_count),
                xytext=(0, min_count - 20),  # 调整标注的位置
                arrowprops=dict(facecolor='black', shrink=0.05),
                ha='center')
    

    # 添加标题和标签
    plt.title('Boxplot of Taxi Orders with Maximum Taxi ID Annotated')
